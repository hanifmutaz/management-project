  ...
  return (
    <>
      ...
      <div className="filters">
        <div className="seg">
          {[['', 'Semua'], ['office', 'Kantor'], ['freelance', 'Freelance'], ['parttime', 'Part-time'], ['personal', 'Personal']].map(([v, l]) =>
              ...
              <div className="pcard" key={p.id} onClick={() => nav('/projects/' + p.id)}>
                ...
                <div className="cardact">
                  <span className="rowact">
                    <button onClick={(e) => { e.stopPropagation(); pinProject(p.id); }} title={p.pinned ? 'Unpin' : 'Pin'} style={p.pinned ? { color:'var(--brand)', borderColor:'var(--brand)' } : undefined}><Icon name="pin" /></button>
                    <button onClick={(e) => { e.stopPropagation(); open(<ProjectForm edit={p} />); }} title="Edit"><Icon name="edit" /></button>
                  </span>
                </div>
                <div className="code"><span className={`badge ${typeClass[p.type]}`}><Icon name={typeIcon[p.type]} size="sm" />{typeLabel[p.type]}</span><span className={`badge ${projStatusClass[p.status]}`}>{projStatusLabel[p.status]}</span></div>
                <div className="name">{p.pinned && <span className="pinico"><Icon name="pin" size="sm" /></span>}{p.name}</div>
                {p.client_name && <div className="cli"><Icon name="user" size="sm" /> {p.client_name}</div>}
                <div className="pbar"><i style={{ width: pct + '%', background: p.color }} /></div>
                <div className="stats">

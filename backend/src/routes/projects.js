...
r.post('/', ah(async (req, res) => {
  ...
  checkEnum(b.type, 'Type', TYPES);
  checkEnum(b.status, 'Status', STATUSES);
  checkEnum(b.rate_type, 'Rate type', RATE_TYPES);
  checkNumber(b.rate, 'Rate', { notNegative: true });
...
r.patch('/:id/pin', ah(async (req, res) => {
  const p = (await query('UPDATE projects SET pinned=NOT pinned WHERE id=$1 RETURNING *', [req.params.id])).rows[0];
  if (!p) throw new ApiError(404, 'Project not found');
  res.json(p);
}));
